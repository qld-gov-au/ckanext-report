...
def report_view(report_name, organization=None, refresh=False):
    ...
    org_in_params = request.values.get('organization')
    if org_in_params:
    ...
    # options
    options = Report.add_defaults_to_options(request.values, report['option_defaults'])
    ...
    # handy for testing
    try:
        refresh = t.asbool(request.values.get('refresh'))
